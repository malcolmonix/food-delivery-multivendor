...
// 1. Add UpdateRestaurantInput after OrderInput
const inputTypeToAdd = `
...
const updatedContent1 = content.replace(
  /(  input OrderInput \{[\s\S]*?\n  \})/,
  `$1\n${inputTypeToAdd}`
);

// 2. Add mutations after placeOrder
...
const updatedContent2 = updatedContent1.replace(
  /(placeOrder\([\s\S]*?instructions: String\n    \): Order!)/,
  `$1${mutationsToAdd}`
);

// 3. Add resolvers after ownerLogin
...
const updatedContent3 = updatedContent2.replace(
  /(ownerLogin: async[\s\S]*?name: admin\.name \|\| 'Admin'\n      \};\n    },)/,
  `$1${resolversToAdd}`
);

writeFileSync(schemaPath, updatedContent3, 'utf-8');
console.log('✅ Schema updated with CRUD mutations');
